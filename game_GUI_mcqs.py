...
def ans():
    total = 0
    if ans1.get().lower() == "shoaib akhtar":
        total += 5
    if ans2.get().lower() == "python":
        total += 5
    if ans3.get().lower() == "burj khalifa":
        total += 5
    if ans4.get().lower() == "h++":
        total += 5
        
    if ans5.get().lower() == "would":
        total += 5
        
    messagebox.showinfo(" Result", f"Your total points is {total}.")
app = tk.Tk()
...
tk.Button(frame, text="Submit", command=ans, bg="lightblue").grid(row=1,column=1,padx=10, pady=10)
